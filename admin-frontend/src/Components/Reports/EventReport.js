...
const PaymentReport = () => {
  ...
  const handleExportToExcel = () => {
    const exportData = filteredPayments.map((payment, index) => ({
      "Sr.No": index + 1,
      "Client Name": payment.enquiry?.client_id?.full_name || "N/A",
  ...
  return (
              ...
              {paginatedEnquiries.length > 0 ? (
                paginatedEnquiries.map((item, index) => (
                  ...
                  >
                    ...
                    <td className="px-3    w-36 py-2text-left text-2xs font-medium text-black tracking-wider">
    ...
    {item.enquiry?.event_name && item.enquiry?.event_name.length > 11
      ...
      : item.enquiry?.event_name || "N/A"}
                    ...
                    <span title={item.enquiry?.event_venue || "N/A"}>
    {item.enquiry?.event_venue && item.enquiry?.event_venue.length > 11
      ? item.enquiry?.event_venue.slice(0, 11) + "..."
      : item.enquiry?.event_venue || "N/A"}
  </span>
                    </td>
                    <td className="px-3    w-36 py-2 text-left text-2xs font-medium text-black tracking-wider">
